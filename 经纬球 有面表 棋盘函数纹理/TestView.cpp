...
CTestView::CTestView()
{
	...
	Alpha = 0.0, Beta = 0.0;
	bPlay = FALSE;
	sphere.SetParameter(300);
	sphere.ReadVertex();//读入顶点
	sphere.ReadFacet();//读入曲面
	transform.SetMatrix(sphere.V, sphere.nVertex);
...
void CTestView::InitializeLightingScene(void)//初始化光照场景
{
	...
	pMaterial->SetAmbient(CRGBA(0.847, 0.10, 0.075));//环境反射率
	pMaterial->SetDiffuse(CRGBA(0.852, 0.006, 0.026));//漫反射率
	pMaterial->SetSpecular(CRGBA(1.0, 1.0, 1.0));//镜面反射率
